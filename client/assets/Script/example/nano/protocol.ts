...
export default class Protocol {
     ...
     * @param length
     */
    static copyArray(dest: any, doffset: number, src: any, soffset: number, length: number) {
    ...
    static msgHasId(type: number) {
        return type === Message.TYPE_REQUEST || type === Message.TYPE_RESPONSE;
    }

    static msgHasRoute(type: number) {
        return type === Message.TYPE_REQUEST || type === Message.TYPE_NOTIFY ||
            type === Message.TYPE_PUSH;
    }

    static caculateMsgIdBytes(id: number) {
        var len = 0;
        do {
            len += 1;
            id >>= 7;
    ...
    static encodeMsgFlag(type: number, compressRoute: number, buffer: any, offset: number) {
    ...
    static encodeMsgId(id: number, buffer: any, offset: number) {
        do {
            var tmp = id % 128;
    ...
    static encodeMsgRoute(compressRoute: number, route: any, buffer: any, offset: number) {
        if (compressRoute) {
    ...
    static encodeMsgBody(msg: any, buffer: any, offset: number) {
